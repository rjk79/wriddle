...
const Board = () => {
  ...
  return (
    <div className="game">
      ...
      {feedback === GAME_LOST && <div style={{ color: 'red' }}>{`The word was '${answer}'.`}</div>}
      ...
      <div className="border border-black p-5 my-5">
        <div className="font-bold text-base">High Scores</div>
        {scores.map((score, index) => (
          <div key={index}>
            ...
            {index === 2 && <>🥉</>}
            <span className="ml-2">
              <span className="font-bold">{score.name}</span>: {score.value}
            </span>
          </div>
        ))}
      </div>
      ...
      {showInstructions ? (
        <div className="instructions">
          ...
          <div className="hide" onClick={() => setShowInstructions(false)}>
            X
          </div>
          <div>
      ...
      <div className="flex">
        <a className="social-link" href="https://github.com/rjk79" target="_blank" rel="noreferrer">
          <img src="github.png" />
        </a>
        <a
